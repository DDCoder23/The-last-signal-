...
fn test_error_channels() {
    if let Some(value) = helpers::dispatch(COUNT) {
        work(value)
    }
}
...
#[cfg(feature = "async")]
fn work(value: u8) {
    use flexi_logger::{test_utils, ErrorChannel, FileSpec, Logger, WriteMode};
    use log::*;
    use std::{
        fs::File,
    ...
    let err_file = test_utils::child_in_dir("flexi_logger_error_channel.err");
    match value {
        0 => {
